 ...
 * ================================
 */
const DEFAULT_PORT = parseInt(process.env.PORT) || 9699;
...
const CONFIG = {
  ...
  HEALTH_CHECK_PORT: parseInt(process.env.HEALTH_CHECK_PORT) || DEFAULT_PORT,
...
class FileManager {
  ...
  static async validateBotFile() {
    ...
    try {
      ...
      Logger.error(
        "telegram.js not found! Please ensure it exists in the root directory."
      );
...
class KeepAliveService {
  ...
  setupKeepAlive() {
      ...
      async () => {
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            Logger.warning(
              `Keep-alive ping returned status: ${response.status}`
            );
          }
        } catch (error) {
    ...
    Logger.success(
      `Keep-alive service configured (every ${CONFIG.KEEP_ALIVE_INTERVAL} minutes)`
    );
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.server.on("error", (err) => {
        if (err.code === "EADDRINUSE") {
          Logger.warning(
            `Port ${this.port} is already in use, assuming server is running`
          );
          this.isStarted = true;
          resolve();
        } else {
...
class BotProcessManager {
  ...
  scheduleRestart() {
    ...
    if (this.restartCount > CONFIG.MAX_RESTARTS) {
      Logger.error(
        `Too many restarts (${this.restartCount}). Waiting before trying again...`
      );
  ...
  async cleanupPM2() {
    try {
      ...
      spawnSync(
        npmCommand.command,
        [...npmCommand.pm2Args, "delete", CONFIG.APP_NAME],
        {
          stdio: "ignore",
        }
      );
  ...
  async handlePM2Process(pm2Process) {
    return new Promise((resolve, reject) => {
      ...
      pm2Process.stderr.on("data", (data) => {
        ...
        if (output.includes("restart")) {
          ...
          if (restartCount > CONFIG.MAX_RESTARTS) {
            Logger.warning(
              "PM2 restarting too frequently, falling back to direct execution"
            );
            pm2Process.kill();
      ...
      pm2Process.on("exit", (code) => {
        if (code !== 0) {
          Logger.warning(
            `PM2 exited with code ${code}, falling back to direct execution`
          );
          this.startMethod = "direct";
          this.startDirectly().then(resolve).catch(reject);
        } else {
...
╚═══════════════════════════════════════════════════════════════╝
  ...
  async initialize() {
    try {
      ...
      Logger.info(
        `💓 Keep-alive: Active (${CONFIG.KEEP_ALIVE_INTERVAL}min intervals)`
      );
  ...
  async shutdown() {
    ...
    try {
      await Promise.all([this.webServer.shutdown(), this.botProcess.kill()]);
